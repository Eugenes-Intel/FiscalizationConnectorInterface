namespace Rootsware.Fiscalization.Integration.FCI.Helpers;
...
internal class Helper
{
    internal static T ScopeEntity<T>(string entityType) where T : class
    {
        ...
        if (Activator.CreateInstance(assemblyName, entityType)?.Unwrap() is not T entity) throw new EntryPointNotFoundException(nameof(FCI));
    ...
    internal static T ScopeEntity<T>() where T : class
    {
        ...
        if (instance is not null)
            if (Activator.CreateInstance(instance) is T t)
                return t;
        throw new EntryPointNotFoundException(nameof(FCI));
    }
}